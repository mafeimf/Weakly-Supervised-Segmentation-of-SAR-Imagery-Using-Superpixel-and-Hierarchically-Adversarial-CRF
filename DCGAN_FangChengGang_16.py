...
#             nn.BatchNorm2d(self.output_dim),
    ...
    def forward(self, input):
        x = self.fc(input)
        x = x.view(-1, 128, (self.input_height_small // 4), (self.input_width_small // 4))
        x = self.deconv(x)
...
class discriminator(nn.Module):
    ...
    def __init__(self, dataset = 'mnist'):
        ...
        self.dc_small = nn.Sequential(     
            nn.Linear(1024, self.output_dim),
            nn.Sigmoid(),
        )
        self.cl_small = nn.Sequential(
...
#             nn.Sigmoid(),
    ...
    def forward(self, input):
        x = self.conv_small(input)
        x = x.view(-1, 128 * (self.input_height_small // 8) * (self.input_width_small // 8))
        x = self.fc_small(x)
        d = self.dc_small(x)
        c = self.cl_small(x)
...
class DCGAN_1G_Class_MSTAR_16(object):
...
#         utils.generate_animation(self.result_dir + '/' + self.dataset + '/' + self.model_name + '/' + self.model_name, self.epoch)
    ...
    def save(self):
        ...
        torch.save(self.D.state_dict(), os.path.join(save_dir, self.model_name + '_D_6trainImg_5class_16.pkl'))
